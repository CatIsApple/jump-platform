...
from ...base import STATUS_COOLDOWN, STATUS_FAILED, STATUS_LOGIN_REQUIRED, STATUS_SUCCESS
...
class OploveSite(GnuboardSite):
    ...
    def jump(self) -> JumpResult:
        ...
        # shop 페이지에서 wr_id 추출
        self.driver.get(f"{self.base_url}/bbs/board.php?bo_table=shop")
        time.sleep(2)
        ...
        self.emit(f"[오피러브] wr_id: {wr_id}", "INFO")

        # 점프 실행
        self.driver.get(f"{self.base_url}/run.jump.php?wr_id={wr_id}")
        ...
        try:
            ...
            alert_obj.accept()
            self.emit(f"[오피러브] Alert: {alert_text}", "INFO")
        except Exception:
            time.sleep(2)
            try:
